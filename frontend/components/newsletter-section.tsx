    ...
    if (email) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          toast.error(to("subscriptionFailed"));
        }
        toast.success(to("subscriptionSuccess"), to("staySafeAlert"), 4000);
      } catch (error) {
        toast.error(
          error instanceof Error ? error.message : to("subscriptionFailed")
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <section className="w-full px-4 sm:px-6 lg:px-8 py-12 bg-primary">
      <div className="max-w-2xl mx-auto text-center">
        ...
        <p className="text-primary-foreground opacity-90 mb-8">{t("desc")}</p>
